...
public class SettingsActivity extends PreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        findPreference("bntInputPref").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                try {
                    SettingsActivity.this.startActivityForResult(new Intent(SettingsActivity.this, InputConfigActivity.class), 3);
                    return true;
                } catch (Exception e) {
        ...
        findPreference("bntCustomKeys").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                try {
                    SettingsActivity.this.startActivityForResult(new Intent(SettingsActivity.this, KeyboardConfigActivity.class), 5);
                    return true;
                } catch (Exception e) {
                    Log.e("SettingsActivity", "EXCEPTION: " + e.toString());
                    return true;
                ...
                public boolean onPreferenceClick(Preference preference) {
                    if (((CheckBoxPreference) preference).isChecked()) {
                        ...
                        if (!new File(str).exists()) {
                            new AlertDialog.Builder(SettingsActivity.this).setTitle(SettingsActivity.this.getString(R.string.settingGameGenieNotFoundTitle)).setMessage(SettingsActivity.this.getString(R.string.settingGameGenieNotFoundSummary) + ": " + str).setPositiveButton(SettingsActivity.this.getString(R.string.buttonOk), (DialogInterface.OnClickListener) null).show();
                            ((CheckBoxPreference) preference).setChecked(false);
                        }
                        if (PreferenceManager.getDefaultSharedPreferences(SettingsActivity.this.getApplicationContext()).getBoolean("enableAutosave", false)) {
                            Toast.makeText(SettingsActivity.this.getApplicationContext(), SettingsActivity.this.getString(R.string.settingGameGenieCantAutoload), 1).show();
                        }
                    }
                    return false;
                }
        ...
        ((CheckBoxPreference) findPreference("enableAutosave")).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                if (((CheckBoxPreference) preference).isChecked() && PreferenceManager.getDefaultSharedPreferences(SettingsActivity.this.getApplicationContext()).getBoolean("useGameGenie", false)) {
                    Toast.makeText(SettingsActivity.this.getApplicationContext(), SettingsActivity.this.getString(R.string.settingGameGenieCantAutoload), 1).show();
                }
                return false;
            }
    ...
    public void onStart() {
        super.onStart();
        for (String findPreference : PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getAll().keySet()) {
